  ...
  try {
    ...
    if (walletData.image) {
      ...
      if (!imageUploadRes.success) {
        return {
          success: false,
          msg: imageUploadRes.msg || "Ошибка в загрузке изображения кошелька",
        };
      }
...
export const deleteTransactionsByWalletId = async (
  ...
  try {
    ...
    while (hasMoreTransactions) {
      ...
      //   console.log(
      //     `${transactionsSnapshot.size} транзакции удаленые в этой партии`
      //   );
    }

    return {
      success: true,
      msg: "Все транзакции удалены успешно",
    };
